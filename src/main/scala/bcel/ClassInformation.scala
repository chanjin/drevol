...
object ClassInformation {
  ...
  def getConstPoolClasses(clazz: JavaClass): List[String] = {
    ...
    depvisitor.start
    depvisitor.getClassNames.toList.filter(c => c != clazz.getClassName && !ClassAnalysisRule.isExcluded(c))
  }
  ...
  def getStructuralDependents(clazz: JavaClass): List[String] = {
    ...
    deps.filter(c => c != clazz.getClassName && !ClassAnalysisRule.isExcluded(c))
  }
